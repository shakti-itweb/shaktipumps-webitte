...
const getfetchProductsForListing = async (req, res) => {
  ...
  try {
    ...
    if (pageType == "controllers") {
      ...
      const queryResponse = await db
        .request()
        .query(`select * from specialApplication `);
      console.log(queryResponse);
      res.render("product/product-listing", queryResponse);
      return; //This will prevent from multiple rendering
    }
...
const getfetchProductDetails = async (req, res) => {
  ...
  try {
    if (table == "pumps") {
      ...
      const queryResponse = await db
        .request()
        .query(`select * from pumpTable where id=${id}`);
      console.log(queryResponse);
      ...
      const queryResponse = await db
        .request()
        .query(`select * from solarPump where id=${id}`);
      console.log(queryResponse);
      res.render("product/product-details", queryResponse);
      return; //This will prevent from multiple rendering
    }
    if (table == "controllers") {
      ...
      const queryResponse = await db
        .request()
        .query(`select * from inverter where id=${id}`);
      console.log(queryResponse);
      res.render("product/product-details", queryResponse);
      return; //This will prevent from multiple rendering
      ...
      const queryResponse = await db
        .request()
        .query(`select * from specialApplication where id=${id}`);
      console.log(queryResponse);
      res.render("product/product-details", queryResponse);
      return; //This will prevent from multiple rendering
    }
